...
def loaddata(filename, ifo=None, tvec=True, readstrain=True, strain_chan=None, dq_chan=None, inj_chan=None):
    ...
    for flag in shortnameList:
        ...
        dq_channel_dict[flag] = (qmask >> bit) & 1
    ...
    for flag in injnameList:
        ...
        inj_channel_dict[flag] = (injmask >> bit) & 1
    ...
    if tvec:
        return strain, time, channel_dict, dq_channel_dict, inj_channel_dict
    else:
        return strain, meta, channel_dict, dq_channel_dict, inj_channel_dict
...
class FileList():
    ...
    def findfile(self, gps, ifo):
        ...
        if len(filenamelist) == 0:
            print(
                "WARNING!  No file found for GPS {0} and IFO {1}".format(gps, ifo))
            return None
        else:
...
def getstrain(start, stop, ifo, filelist=None, strain_chan=None, dq_chan=None, inj_chan=None):
    ...
    segList = getsegs(start, stop, ifo, flag='DATA', filelist=filelist,
                      strain_chan=strain_chan, dq_chan=dq_chan, inj_chan=inj_chan)
    sl = segList.seglist
    ...
    for time in gpsList:
        ...
        strain, meta, dq = loaddata(
            filename, ifo, tvec=False, strain_chan=strain_chan, dq_chan=dq_chan, inj_chan=inj_chan)
        if len(m_strain) == 0:
            m_start = meta['start']
            dt = meta['dt']
        m_stop = meta['stop']
