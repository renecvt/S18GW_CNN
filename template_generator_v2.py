...
def template_generator(ts_list, masses, save):
    approximants = ['SEOBNRv4', 'SEOBNRv2_opt', 'IMRPhenomC']
    for approximant in approximants:
        for ts in ts_list:
            ...
            #     for mass in masses:
            #         # try:
            #         #     plus_polarization, _ = get_td_waveform(approximant = approximant, mass1 = mass, mass2 = mass, delta_t = ts.delta_t, f_lower = 150)
            #         # except:
            #         #     continue
            #         # plus_polarization.resize(len(ts))
            #         template = ts.cyclic_time_shift(ts.start_time)
...
template_generator(ts_list = h1, masses = masses, save = True)